...
烘焙窗口和 `Glass Bake Target` Inspector 都内置“工具预设库”。预设库固定扫描 `Assets/SHADER/玻璃预设/*.json`，下拉列表第一项是 `毛玻璃（内置）`，对应 `Assets/SHADER/玻璃预设/毛玻璃.json`。选择后点击“应用所选预设”即可一次恢复输出尺寸、圆角、透明度、边框、光泽以及全部实时 Shader 参数；烘焙窗口还会恢复预览背景类型和 Assets 内的自定义背景引用。

- `创建并安装当前预设`：直接把当前参数保存到预设库，保存完成后会成为下拉列表的可选项。
- `安装外部 JSON`：选择电脑任意位置的兼容预设，工具会验证并复制到预设库；同名文件会生成安全的新名称，不覆盖已安装预设。
- `刷新预设列表`：在资源管理器中手动加入 JSON 后刷新列表。
- `导出预设 JSON / 读取但不安装`：用于分享或临时试用，仍支持工程外的任意文件夹。

如果 `毛玻璃.json` 不存在，工具会自动用内置标准参数重新创建，因此它会始终作为第一个可选预设存在。

点击“烘焙透明 PNG”后会打开普通系统保存窗口，可输出到 Unity 工程外的任意文件夹，不再限制于 `Assets/`。保存在当前工程 `Assets/` 内时，工具仍会自动导入为 Unity Sprite；保存到外部目录时，会直接生成 RGBA PNG 并在资源管理器中定位该文件。工具会记住上一次的输出目录，图片外部区域 Alpha 为 0。
